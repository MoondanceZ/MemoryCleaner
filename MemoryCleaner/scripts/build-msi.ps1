...
$project = Join-Path $root "MemoryCleaner.csproj"
$installer = Join-Path $root "Installer\Product.wxs"
$artifacts = Join-Path (Split-Path -Parent $root) "artifacts"
...
function Get-ProjectVersion {
    ...
    if ([string]::IsNullOrWhiteSpace($version)) {
        throw "MemoryCleaner.csproj does not contain a <Version> value."
    }
...
if (-not $SkipPublish) {
    Write-Host "Publishing MemoryCleaner AOT ($Configuration, $Runtime)..."
...
$msi = Join-Path $artifacts "MemoryCleaner-$version-x64.msi"
